...
contract ETERNITY is ERC20, Ownable {
    ...
    event DonationEnabledUpdated(bool enabled);
    ...
    event DonationWalletUpdated(address indexed newDonationWallet, address indexed oldDonationWallet);
    ...
    function setDonationEnabled(bool _enabled) external onlyOwner {
        require(donationEnabled != _enabled, "Can't set flag to same status");
        if (_enabled == false) {
            previousMarketingFee = donationFee;
            donationFee = 0;
            donationEnabled = _enabled;
        } 
 
        emit DonationEnabledUpdated(_enabled);
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            uint256 tmpDonationRewardPercent;
            ...
            tmpDonationRewardPercent = amount.mul(donationFee).div(100);
            ...
            fees = tmpMarketingRewardPercent.add(tmpDonationRewardPercent).add(tmpBusdDividedRewardsInContract).add(tmpLpRewardInContract);
            ...
            super._transfer(address(this),donationWallet,tmpDonationRewardPercent);
        }
